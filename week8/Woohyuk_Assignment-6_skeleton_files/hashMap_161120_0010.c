...
void resizeTable(HashMap* map, int capacity)
{
    ...
    //		printf("%d,inside_resize_function\n",__LINE__);
    		...
    		//hashMapPut(pNewHashMap, pLink->key, pLink->value);
    //		 printf("%d,inside_resize_function\n",__LINE__);
    	

    	#if 0
    		if(pNewHashMap->table[newIdx]!=NULL)
    		{
    			hashMapPut(pNewHashMap, pLink->key, pLink->value);
    		}
    		else
    			pNewHashMap->table[newIdx] = pLink;
    		pLink = pLink->next;
    	#endif
    		//pLink = pLink->next;
    	//	 printf("%d,inside_resize_function\n",__LINE__);
		...
		oldMap->table = map->table;
		 printf("%d,inside_resize_function\n",__LINE__);
		map->table = pNewHashMap->table;
		 printf("%d,inside_resize_function\n",__LINE__);
	    hashMapCleanUp(oldMap);
	    printf("\nend of resize function being used\n");
	#endif
	free(oldTable);
	map->capacity = capacity;
	map->size = size;
