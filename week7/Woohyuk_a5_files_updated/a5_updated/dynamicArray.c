...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
    ...
    int leftChild = 2*position + 1; int rightChild = 2*position + 2;
    int min;
    if(rightChild < last) // there are 2 child nodes
    {
        /*getting the child node whose priority is smaller*/
        if(compare(heap->data[leftChild], heap->data[rightChild]) > 0)
         {
             min = rightChild;
         }   
        else
        ...
        if(compare(heap->data[min], heap->data[position] ) < 0)
        {
            dySwap(heap, min, position);
            adjustHeap(heap, last, min, compare);
        }   
    }
    else if(leftChild < last) // there is only one child node
    {
        if(compare(heap->data[leftChild], heap->data[position] ) < 0)
        {
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
    ...
    while(pos!=0)
    {
       if(compare(value, heap->data[(pos-1)/2]) < 1)
       {
            dySwap(heap, pos, (pos-1)/2);
            pos = (pos-1)/2;
       }
       else
        return;
...
void dyHeapRemoveMin(DynamicArray* heap, compareFunction compare)
{
    ...
    int pos = dySize(heap);
    dySwap(heap, 0, pos-1);
    dyRemoveAt(heap, pos-1);
    adjustHeap(heap, pos-1, 0, compare);
}
