...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    // FIXME: Implement
    clearVisited(graph);
    Deque* newStack = dequeNew();
    //int start = source->label;
   // graph->vertexSet[start].isVisited = 1;
    dequePushFront(newStack, source);

    while(!dequeIsEmpty(newStack))
    {
        Vertex* front = dequeFront(newStack);
        dequePopFront(newStack);
        if(!graph->vertexSet[front->label].isVisited)
        {
            graph->vertexSet[front->label].isVisited = 1;   
            
            for(int i = 0; i < front->numNeighbors; i++)
            {
                if(!front->neighbors[i]->isVisited)
                    dequePushBack(newStack, front->neighbors[i]);            
            }
        }

    }

    int end = destination->label;
    if(graph->vertexSet[end].isVisited)
    {
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
    // FIXME: Implement
    clearVisited(graph);
    Deque* newQueue = dequeNew();
    //int start = source->label;
   // graph->vertexSet[start].isVisited = 1;
    dequePushBack(newQueue, source);

    while(!dequeIsEmpty(newQueue))
    {
        Vertex* front = dequeFront(newQueue);
        dequePopFront(newQueue);
        if(!graph->vertexSet[front->label].isVisited)
        {
            graph->vertexSet[front->label].isVisited = 1;   
            ...
            {
                if(!front->neighbors[i]->isVisited)
                    dequePushBack(newQueue, front->neighbors[i]);            
            }
        }
