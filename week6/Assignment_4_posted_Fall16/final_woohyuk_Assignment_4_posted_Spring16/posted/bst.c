...
/* Name: Woohyuk Yang
 * Date: Oct. 27th ,2016
 * Solution description: function library for binary search tree. 
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	/*write this*/
		...
		assert(val!=NULL);
		if(compare(cur->val, val) == 0)
			if((cur->left == NULL)&&(cur->right==NULL))//when there is only one child node
			{
				_freeBST(cur);
				return NULL;
			}
...
int main(int argc, char *argv[]){	
   ...
   //After implementing your code, please uncommnet the following calls to the test functions and test your code 
    ...
    /*My own test*/    
	struct BSTree* newTree = buildBSTTree();
	printTree(newTree);
